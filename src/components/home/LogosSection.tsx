...
export function LogosSection() {
  ...
  return (
    <section className="py-16 overflow-hidden bg-surface-light border-y border-surface-light-border">
      <div className="container-wide">
        <p className="text-center text-xs text-surface-light-muted uppercase tracking-widest mb-10 font-medium">
          {t('title')}
        </p>
      </div>
        ...
        <div className="flex w-max animate-scroll-logos">
          {duplicated.map((logo, index) => (
            <div key={index} className="flex-shrink-0 mx-4">
              <div className="relative h-24 w-56 rounded-xl bg-white border border-surface-light-border shadow-sm flex items-center justify-center px-6 transition-shadow duration-300 hover:shadow-md">
                <Image
                  src={logo.src}
                  alt={logo.name}
                  fill
